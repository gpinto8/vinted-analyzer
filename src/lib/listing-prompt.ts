...
export function buildAnalysisPrompt(input: AnalyzeListingInput): string {
  ...
  return [
    ...
    "",
    '4. PRODUCT TYPE: The specific garment type as buyers search for it (e.g. "Oversized hoodie", "Slim fit chinos", "Crop cardigan", "V-neck t-shirt"). Use the same specific wording as in the title — not a generic word like "top". If the seller gave a product type hint, use or refine it; otherwise infer from the photos.',
    "",
    "5. BRAND: Identify from labels, logos, tags visible in the photos. Cross-check with seller hint. Use hint or empty string if unidentifiable.",
    "",
    "6. SIZE: Read from tags/labels. Use XS/S/M/L/XL/XXL or numeric (36, 38, 40, 42). Estimate from proportions if not visible.",
    "",
    '7. COLOR: Primary color as buyers search it (e.g. "Dark grey", "Navy blue", "Cream", "Burgundy").',
    "",
    '8. MATERIAL: From care label or visual texture. Be specific (e.g. "100% Cotton", "80% Cotton 20% Polyester", "Wool blend").',
    "",
    "9. MEASUREMENTS: Return as a JSON object with measurement names as keys and cm values as numbers (no units in values).",
    "   By garment type:",
    ...
    "",
    '10. TAGS: 5-8 lowercase search keywords buyers would use (e.g. ["nike", "hoodie", "vintage", "grey", "streetwear", "oversized", "cotton"]).',
    "",
    "11. PRICING (numbers only, in EUR):",
...
export function mapToListingResult(data: Record<string, unknown>): ListingResult {
  return {
    ...
    productType: String(data.productType ?? data.product_type ?? "").trim() || undefined,
